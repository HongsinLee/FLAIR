...
class AdaADnoBuffer(ContinualModel):
    NAME = 'AdaADnoBuffer'
    COMPATIBILITY = ['class-il', 'task-il']

    def __init__(self, backbone, loss, args, transform):
        super(AdaADnoBuffer, self).__init__(backbone, loss, args, transform)
    ...
    def robust_observe(self, inputs, labels, not_aug_inputs, num_class, logits=None):
        ...
        if self.task == 0:
            ...
            self.old_net.eval()
            with torch.no_grad():
                old_output_plus = self.old_net(inputs_adv)[:, :pc]
            criterion_kl = nn.KLDivLoss(reduction="batchmean")
            distill_loss = self.args.alpha * criterion_kl(F.log_softmax(outputs_plus[:, :pc], dim=1), F.softmax(old_output_plus.detach(), dim=1))
            targets = self.eye[labels][:, :ac]
