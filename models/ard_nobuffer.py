...
class ARDnoBuffer(ContinualModel):
    NAME = 'ARDnoBuffer'
    COMPATIBILITY = ['class-il', 'task-il']

    def __init__(self, backbone, loss, args, transform):
        super(ARDnoBuffer, self).__init__(backbone, loss, args, transform)
    ...
    def robust_observe(self, inputs, labels, not_aug_inputs, num_class, logits=None):
        ...
        if self.task == 0:
            ...
            distill_loss = self.args.alpha * criterion_kl(F.log_softmax(outputs_plus[:, :pc], dim=1), F.softmax(old_output.detach(), dim=1))
            targets = self.eye[labels][:, :ac]
