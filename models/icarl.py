...
class ICarl(ContinualModel):
    ...
    def robust_observe(self, inputs, labels, not_aug_inputs, num_class, logits=None):
        ...
        if self.task == 0:
            ...
            loss = (1 - pc/ac) *F.binary_cross_entropy_with_logits(outputs_adv[:, pc:ac], targets) + self.args.alpha * (pc/ac) *F.binary_cross_entropy_with_logits(outputs[:, :pc], logits[:, :pc])
            assert loss >= 0
    ...
    def get_loss(self, inputs: torch.Tensor, labels: torch.Tensor,
        ...
        if task_idx == 0:
            ...
            targets = self.eye[labels][:, pc:ac]
            loss = F.binary_cross_entropy_with_logits(outputs[:, pc:ac], targets) + self.args.alpha * F.binary_cross_entropy_with_logits(outputs[:, :pc], logits[:, :pc])
            assert loss >= 0
