...
def lucir_batch_hard_triplet_loss(labels, embeddings, k, margin, num_old_classes):
    ...
    if hard_num > 0:
        ...
        loss = nn.MarginRankingLoss(margin=margin)(gt_scores.view(-1, 1),
                                                   max_novel_scores.view(-1, 1), torch.ones(hard_num * k).view(-1,1).to(embeddings.device))
    else:
...
class Lucir(ContinualModel):
    ...
    def robust_observe(self, inputs, labels, not_aug_inputs, num_class, logits=None, epoch=None, fitting=False):
        if not hasattr(self, 'classes_so_far'):
            self.register_buffer('classes_so_far', labels.unique().to('cpu'))
        else:
            self.register_buffer('classes_so_far', torch.cat((
        ...
        self.opt.zero_grad()
        loss = self.get_loss(
            inputs_adv, labels.long(), self.task)
        loss.backward()
    ...
    def end_task(self, dataset) -> None:
        ...
        self.lamda_cos_sim = math.sqrt(
            self.task +1) * float(self.args.lamda_base)
