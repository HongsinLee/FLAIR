...
class LWF(ContinualModel):
    ...
    def robust_observe(self, inputs, labels, not_aug_inputs, num_class, logits=None):
        ...
        if self.task == 0:
            loss = XNET_loss(outputs_plus[:, :ac], labels)
        else:
            self.old_net.eval()
            with torch.no_grad():
            ...
            distill_loss = self.args.alpha * criterion_kl(F.log_softmax(outputs[:, :pc], dim=1), F.softmax(old_output.detach(), dim=1))
            loss = XNET_loss(outputs_plus[:, :ac], labels) + distill_loss
