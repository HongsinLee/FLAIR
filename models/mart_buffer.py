...
class MARTBuffer(ContinualModel):
    NAME = 'MARTBuffer'
    COMPATIBILITY = ['class-il', 'task-il']

    def __init__(self, backbone, loss, args, transform):
        super(MARTBuffer, self).__init__(backbone, loss, args, transform)
    ...
    def robust_observe(self, inputs, labels, not_aug_inputs, num_class, logits=None):
        ...
        logits = self.net(inputs)[:, :ac]
        logits_adv = self.net(inputs_adv)[:, :ac]

        adv_probs = F.softmax(logits_adv, dim=1)

        tmp1 = torch.argsort(adv_probs, dim=1)[:, -2:]
        new_y = torch.where(tmp1[:, -1] == labels, tmp1[:, -2], tmp1[:, -1])
        loss_adv = F.cross_entropy(logits_adv, labels) + F.nll_loss(torch.log(1.0001 - adv_probs + 1e-12), new_y)
        ...
        loss = loss_adv + self.args.alpha * loss_robust
