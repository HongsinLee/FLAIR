...
class RSLADnoBuffer(ContinualModel):
    NAME = 'ARDnoBuffer'
    COMPATIBILITY = ['class-il', 'task-il']

    def __init__(self, backbone, loss, args, transform):
        super(RSLADnoBuffer, self).__init__(backbone, loss, args, transform)
    ...
    def robust_observe(self, inputs, labels, not_aug_inputs, num_class, logits=None):
        ...
        XNET_loss = nn.CrossEntropyLoss()
        self.opt.zero_grad()

        pc = self.task * self.dataset.N_CLASSES_PER_TASK
        ac = (self.task + 1) * self.dataset.N_CLASSES_PER_TASK
        outputs_plus= self.net(inputs_adv)
        if self.task == 0:
            targets = self.eye[labels][:, :ac]
            loss = F.binary_cross_entropy_with_logits(outputs_plus[:, :ac], targets)
        else:
            self.old_net.eval()
            outputs= self.net(inputs)
            with torch.no_grad():
                old_output = self.old_net(inputs)[:, :pc]
            criterion_kl = nn.KLDivLoss(reduction="batchmean")
            distill_loss = self.args.alpha * (5/6 * criterion_kl(F.log_softmax(outputs_plus[:, :pc], dim=1), F.softmax(old_output.detach(), dim=1)) +
                                              1/6 * criterion_kl(F.log_softmax(outputs[:, :pc], dim=1), F.softmax(old_output.detach(), dim=1)))
            targets = self.eye[labels][:, :ac]
            loss = F.binary_cross_entropy_with_logits(outputs_plus[:, :ac], targets) + distill_loss
