...
class TRADESBuffer(ContinualModel):
    NAME = 'TRADESBuffer'
    COMPATIBILITY = ['class-il', 'task-il']

    def __init__(self, backbone, loss, args, transform):
        super(TRADESBuffer, self).__init__(backbone, loss, args, transform)
    ...
    def robust_observe(self, inputs, labels, not_aug_inputs, num_class, logits=None):
        ...
        inputs_adv = TRADES(inputs, labels, self, steps=10)

        outputs_plus= self.net(inputs_adv)[:, :ac]
        outputs = self.net(inputs)[:, :ac]

        loss_natural = F.cross_entropy(outputs, labels)
        loss_robust = criterion_kl(F.log_softmax(outputs_plus, dim=1),
                                                        torch.clamp(F.softmax(outputs, dim=1), min=1e-8))
        loss = loss_natural + self.args.alpha * loss_robust 
        if not (loss >= 0):
            inputs_adv = PGD(inputs, labels, self, steps=10)
            outputs_plus= self.net(inputs_adv)[:, :ac]
            loss = XNET_loss(outputs_plus, labels)
            loss.backward()
            self.opt.step()
            print("TRADES loss error --> PGD-AT")
            return loss.item()
