        ...
        for data in test_loader:
            with torch.no_grad():
                ...
                if 'class-il' not in model.COMPATIBILITY:
                    ...
                    #if hasattr(model, 'get_NN'):
                    #    outputs = model.get_NN(inputs)
                    #else :
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    if not args.nowand:
        ...
        wandb.save(mammoth_path+'/models/'+str(args.model)+'.py', base_path=mammoth_path+'/models')
        wandb.save(mammoth_path+'/utils/training.py', base_path=mammoth_path+'/utils')
    ...
    if not args.ignore_other_metrics:
        ...
        if hasattr(model, 'get_NN') == False and model.NAME != 'pnn':
    ...
    for t in range(dataset.N_TASKS):
        ...
        accs_logit, accs_adv, per_class_output, per_class_output_adv, feat_distance = evaluate_PGD(model, dataset)
        ...
        mean_acc_adv = np.mean(accs_adv)
        mean_acc_logit = np.mean(accs_logit)
        ...
        print("mean_accs_logit = {}, mean_accs_adv  = {}".format(mean_acc_logit, mean_acc_adv))
        if not args.disable_log:
            logger.log(mean_acc)
        ...
        if t == dataset.N_TASKS - 1:
            #if model.NAME != "lucir":
            #    curvatures = evaluate_curvature(model, dataset, args.dataset_mean, args.dataset_std)
            #else:
            #    curvatures = -1
            ...
            accs_adv_AA = evaluate_AA(model, dataset)
            if not args.nowand:
